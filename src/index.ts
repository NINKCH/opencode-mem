...
import type { MemoryScope, MemoryType, MemorySearchResult } from "./types/index.js";
...
export const LocalMemoryPlugin: Plugin = async (ctx: PluginInput) => {
  const { directory } = ctx;
        ...
        for (const provider of response.data.all) {
          if (provider.models) {
            for (const [modelId, model] of Object.entries(provider.models)) {
              if ((model as any).limit?.context) {
                modelLimits.set(`${provider.id}/${modelId}`, (model as any).limit.context);
              }
            }
          }
        }
  ...
  return {
    ...
    tool: {
      "local-memory": tool({
        description:
          "Manage and query the local persistent memory system. Use 'search' to find relevant memories, 'add' to store new knowledge, 'list' to see recent memories, 'forget' to remove a memory.",
        args: {
          mode: tool.schema
              ...
              case "search": {
                ...
                if (scope === "user") {
                  const result = await store.searchMemories(
                    args.query,
                    tags.user,
                    { threshold: CONFIG.similarityThreshold, limit: args.limit || 10 }
                  );
                ...
                const combined = [
                  ...userResult.map((r) => ({ ...r, scope: "user" as const })),
                  ...projectResult.map((r) => ({ ...r, scope: "project" as const })),
